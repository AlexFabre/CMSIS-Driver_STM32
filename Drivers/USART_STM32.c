...
// Detects the version of the HAL being used by the project.
// 'HAL_VERSION_MAIN' was introduced with new HAL2 releases. 
// Therefore it's presence indicates HAL2 functions prototypes must be used.
#if (HAL_VERSION_MAIN >= 2)
#define __HAL2
#endif
...
static int32_t USARTn_Control (const RO_Info_t * const ptr_ro_info, uint32_t control, uint32_t arg) {
  ...
  if (ptr_ro_info->ptr_huart->hdma_rx != NULL) { // If DMA is used for Rx
    if (data_bits == 9U) {
      LL_DMA_SetSrcDataWidth(HAL_DMA_CHANNEL_INSTANCE(ptr_ro_info->ptr_huart->hdma_rx), LL_DMA_SRC_DATA_WIDTH_HALFWORD);
      LL_DMA_SetDestDataWidth(HAL_DMA_CHANNEL_INSTANCE(ptr_ro_info->ptr_huart->hdma_rx), LL_DMA_DEST_DATA_WIDTH_HALFWORD);
    } else {
      LL_DMA_SetSrcDataWidth(HAL_DMA_CHANNEL_INSTANCE(ptr_ro_info->ptr_huart->hdma_rx), LL_DMA_SRC_DATA_WIDTH_BYTE);
      LL_DMA_SetDestDataWidth(HAL_DMA_CHANNEL_INSTANCE(ptr_ro_info->ptr_huart->hdma_rx), LL_DMA_DEST_DATA_WIDTH_BYTE);
    }
  ...
  if (ptr_ro_info->ptr_huart->hdma_tx != NULL) { // If DMA is used for Tx
    if (data_bits == 9U) {
      LL_DMA_SetSrcDataWidth(HAL_DMA_CHANNEL_INSTANCE(ptr_ro_info->ptr_huart->hdma_tx), LL_DMA_SRC_DATA_WIDTH_HALFWORD);
      LL_DMA_SetDestDataWidth(HAL_DMA_CHANNEL_INSTANCE(ptr_ro_info->ptr_huart->hdma_tx), LL_DMA_DEST_DATA_WIDTH_HALFWORD);
    } else {
      LL_DMA_SetSrcDataWidth(HAL_DMA_CHANNEL_INSTANCE(ptr_ro_info->ptr_huart->hdma_tx), LL_DMA_SRC_DATA_WIDTH_BYTE);
      LL_DMA_SetDestDataWidth(HAL_DMA_CHANNEL_INSTANCE(ptr_ro_info->ptr_huart->hdma_tx), LL_DMA_DEST_DATA_WIDTH_BYTE);
    }
...
void HAL_UART_ErrorCallback (HAL_UART_HANDLE_TYPE *huart) {
  ...
  error = HAL_UART_GetLastErrorCodes(huart);
  event = 0U;
